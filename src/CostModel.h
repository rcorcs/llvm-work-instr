...
static int getInstructionCost(TargetTransformInfo *TTI, const Instruction *I){
  if(!TTI || !I) return 0;
  ...
  case Instruction::Alloca: {
    ...
    return TTI->getMemoryOpCost(Instruction::Load, I->getType(),
                                 AI->getAlignment(),
  ...
  case Instruction::Call:
    if(const CallInst *CI = dyn_cast<CallInst>(I)){
       ...
       if(CI->getCalledFunction()==NULL)// || CI->getCalledFunction()->begin()==CI->getCalledFunction()->end())
          return 1;
       return TTI->getCallCost(CI->getCalledFunction(), argsRef);
    }
    /*
...
static int getBasicBlockCost(TargetTransformInfo *TTI, const BasicBlock *BB){
  if(!TTI || !BB) return 0;
  int basicBlockCost = 0;
  for(auto it = BB->begin(); it!=BB->end(); it++){
    int instCost = getInstructionCost(TTI, dyn_cast<Instruction>(&(*it)));
    basicBlockCost += instCost;
  }
