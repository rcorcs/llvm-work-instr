...
public:
  ...
  // when Removed is set.
  std::set<const BasicBlock*> AllBasicBlocks;
  ...
  std::set<const BasicBlock *> getSuccessors(const BasicBlock *Src){
     std::set<const BasicBlock *> S;
     for(auto &E : AllEdges){
        if(E->SrcBB==Src) S.insert(E->DestBB);
     }
     return S;
  }

  std::set<const BasicBlock *> getPredecessors(const BasicBlock *Dest){
     std::set<const BasicBlock *> S;
     for(auto &E : AllEdges){
        if(E->DestBB==Dest) S.insert(E->SrcBB);
     }
     return S;
  }
  

  bool hasEdge(const BasicBlock *Src, const BasicBlock *Dest){
      for(auto &E : AllEdges){
        if((E->SrcBB==Src)&&(E->DestBB==Src))return true;
     }
     return false;
  }
  ...
  void sortEdgesByWeight() {
    std::stable_sort(AllEdges.begin(), AllEdges.end(),
                     [](const std::unique_ptr<Edge> &Edge1,
                       ...
                       /*
                       uint64_t w1 = Edge1->Weight;
                       uint64_t w2 = Edge2->Weight;
                       if(w1!=w2) return w1>w2;
                       else {
                          return reinterpret_cast<std::uintptr_t>(Edge1->SrcBB) > reinterpret_cast<std::uintptr_t>(Edge2->SrcBB);
                       }
                       */
                     });
  }
  ...
  BBInfo *rootGroup(BBInfo *G){
    //errs() << "\tRank: " << G->Rank << " Index: " << G->Index << " Group: " << reinterpret_cast<std::uintptr_t>(G) << "\n";
    while(G!=G->Group){
       G = static_cast<BBInfo *>(G->Group);
    }
    //errs() << "\tRank: " << G->Rank << " Index: " << G->Index << " Group: " << reinterpret_cast<std::uintptr_t>(G) << "\n";
    return G;
  }
  ...
  void computeMinimumSpanningTree() {
    errs() << "Computing Minimum Spanning Tree\n";
      ...
      if (Ei->IsCritical) {
        if (Ei->DestBB && Ei->DestBB->isLandingPad()) {
          if (unionGroups(Ei->SrcBB, Ei->DestBB)){
            ...
            errs() << "Src: " << reinterpret_cast<std::uintptr_t>(Ei->SrcBB) << " Dest: " << reinterpret_cast<std::uintptr_t>(Ei->DestBB) << " : " << Ei->Weight << "\n";
          }
        }
      }
    }
  ...
  Edge &addEdge(const BasicBlock *Src, const BasicBlock *Dest, uint64_t W) {
    AllBasicBlocks.insert(Src);
    AllBasicBlocks.insert(Dest);
